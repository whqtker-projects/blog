...
기존에는 필요한 객체를 애플리케이션 코드가 직접 생성하고 연결하는 경우가 많았기 때문에, 하나의 객체가 다른 객체의 구체적인 구현에 강하게 의존하기 쉬웠습니다. 이런 구조에서는 구현이 바뀌면 관련 코드도 함께 수정해야 했고, 변경의 영향 범위도 커졌습니다. 스프링은 **객체의 생성과 관리를 컨테이너가 담당**하도록 합니다. 이 방식으로 클래스가 필요한 구현체를 직접 생성하지 않고, 외부에서 주입받아 사용할 수 있게 되었습니다. 이 방식을 통해 객체 간 결합을 줄일 수 있고, 더 유연한 구조를 만들 수 있습니다.
