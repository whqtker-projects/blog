...
`spring-framework` 하위의 `spring-aop-and-transactions` 소 시리즈를 `spring-aop`와 `spring-transactions`로 분리했다. AOP 개념과 프록시 기반 AOP 글은 `spring-aop`로, 트랜잭션 추상화·전파/격리·프록시 함정 글은 `spring-transactions`로 옮겼다.

`spring-web-mvc`의 child order는 4로 조정했고, parent series index와 운영 문서, backlog, post metadata, decision record를 새 구조에 맞췄다.
