...
- parent series links use `[[series:<parent>]]`
- child series links use `[[series:<parent>/<child>]]`
- use `|display text` when the reader-facing label should differ from the slug target
- idea-stage post stubs should include at least one series link and at least one related post or concept link so graph connectivity does not depend on frontmatter alone
...
- parent and child series index bodies may include explicit series links for graph-friendly navigation context
- parent series index bodies may link to their child series and to related parent series when useful
- child series index bodies may link to their parent series and sibling or adjacent child series when useful
- series index bodies must not become manual post inventories; the actual post list remains auto-generated by the site
