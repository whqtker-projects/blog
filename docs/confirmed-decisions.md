...
**Last updated:** 2026-05-21 (D-81 updated: Spring AOP and transactions split into separate child series)
...
| D-81 | Spring content is split into two distinct parent directions under the backend/systems domain: `spring-framework` and `spring-boot`. `spring-framework` uses the child series `spring-core`, `spring-aop`, `spring-transactions`, and `spring-web-mvc`; `spring-boot` uses `spring-boot-basics`, `spring-boot-configuration`, and `spring-boot-testing-and-operations`. |
