...
| Field | Type | Description |
|---|---|---|
| `status` | string | Post lifecycle stage. Confirmed values: `idea`, `outline`, `draft`, `review`, `published` (D-30). No default value (D-32). Production build: only `published` → included; absent, `idea`, `outline`, `draft`, `review` → excluded (D-33). |
